...
void bu_Hash(unsigned char *md256, const unsigned char *data, size_t data_len)
{
...
bool bp_privkey_set(struct bp_key *key, const unsigned char *privkey_, size_t pk_len)
{
...
bool bp_pubkey_set(struct bp_key *key, const unsigned char *pubkey_, size_t pk_len)
{
...
bool bp_key_secret_set(struct bp_key *key, const unsigned char *privkey_, size_t pk_len)
{
	bp_key_free(key);
...
bool bp_privkey_get(const struct bp_key *key, unsigned char **privkey, size_t *pk_len)
{
	if (!EC_KEY_check_key(key->k))
		return false;

	size_t sz = i2d_ECPrivateKey(key->k, 0);
	unsigned char *orig_mem, *mem = (unsigned char *)malloc(sz);
	orig_mem = mem;
...
bool bp_pubkey_get(const struct bp_key *key, unsigned char **pubkey, size_t *pk_len)
{
	if (!EC_KEY_check_key(key->k))
		return false;

	size_t sz = i2o_ECPublicKey(key->k, 0);
	unsigned char *orig_mem, *mem = (unsigned char *)malloc(sz*10);
	orig_mem = mem;
...
bool bp_key_secret_get(unsigned char *p, size_t len, const struct bp_key *key)
{
	if (!p || len < 32 || !key)
		return false;
...
bool bp_sign(const struct bp_key *key, const unsigned char *data, size_t data_len,
		unsigned char **sig_, size_t *sig_len_)
{
	size_t sig_sz = ECDSA_size(key->k);
	unsigned char *sig = (unsigned char *)calloc(1, sig_sz);
...
bool bp_verify(const struct bp_key *key, const unsigned char *data, size_t data_len,
	       unsigned char *sig_, size_t sig_len)
{
